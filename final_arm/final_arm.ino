#include <ros.h>
#include <math.h>
#include <final_rover/anglesMsg.h>
...
void subCb(const final_rover::anglesMsg &msg) {
...
ros::Subscriber<final_rover::anglesMsg> sub("/arm_angles_server", &subCb);
...
void setup() {
  ...
  pinMode(CLK_PIN_first, INPUT_PULLUP);
  pinMode(DT_PIN_first, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(CLK_PIN_first), ISR_encoderChange_first, RISING);

  pinMode(CLK_PIN_second, INPUT_PULLUP);
  pinMode(DT_PIN_second, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(CLK_PIN_second), ISR_encoderChange_second, RISING);

  pinMode(CLK_PIN_diff1, INPUT_PULLUP);
  pinMode(DT_PIN_diff1, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(CLK_PIN_diff1), ISR_encoderChange_diff1, RISING);

  pinMode(CLK_PIN_diff2, INPUT_PULLUP);
  pinMode(DT_PIN_diff2, INPUT_PULLUP);
